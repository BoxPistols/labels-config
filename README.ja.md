...
ターミナル完結型 GitHub ラベル管理システム - gh CLI 使用、トークン不要

GitHub ラベルの定義、検証、同期を強力なスキーマベースの設定システムで実現。**gh CLI 認証を使用** - GitHub Token の手動管理は不要。9 種類のビルトインテンプレート、TypeScript でのカスタムラベル定義、ラベル設定ファイルの生成が可能です。

**使用方法:**
- 📦 npm パッケージとしてコード内で使用
- 🛠️ CLI ツールでラベルファイルの生成と検証
- 🔄 gh CLI を使った GitHub 同期ツール（トークン不要 - gh auth 使用）
...
- **ターミナル完結型 gh CLI**: トークン管理不要 - gh CLI 認証を使用
...
- **gh CLI による GitHub 統合**: GitHub リポジトリへのラベル同期（トークン不要）
